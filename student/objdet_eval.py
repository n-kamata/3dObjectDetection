...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            for detection in detections:
                ...
                idx, x, y, z, h, w, l, yaw = detection
                detection_corners = tools.compute_box_corners(x, y, w, l, yaw)
                
                ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                dist = np.array([label.box.center_x, label.box.center_y, label.box.center_z]) - np.array([x, y, z])
                
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                label_polygon = Polygon(label_corners)
                detect_polygon = Polygon(detection_corners)
                intersection = detect_polygon.intersection(label_polygon)
                union = detect_polygon.union(label_polygon)
    ...
    # print("ious: " + str(ious))
    # print("center_devs: " + str(center_devs))
    ####### ID_S4_EX2 START #######     
    #######
    ...
    all_positives = int(labels_valid.sum())
    ...
    # print("det_performcance: " + str(det_performance))
...
def compute_performance_stats(det_performance_all, configs_det):
    ...
    for item in det_performance_all:
        ...
        # print("iou: " + str(itme[0]))
        # print("center_devs: " + str(item[1]))
